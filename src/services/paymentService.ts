...
class PaymentService {
  ...
  async createSubscriptionPaymentIntent(
    ...
    try {
      ...
      const planQuery = SubscriptionPlan.findById(
      ...
      if (!brand.stripeConnectAccountId || !brand.stripeOnboardingComplete) {
        throw this.createError(
          "STRIPE_001",
          "Brand Stripe account not properly configured",
          400
        );
      }
      ...
      if (!canProcessPayments) {
        throw this.createError(
          "STRIPE_002",
          "Brand cannot process payments at this time",
          400
        );
      }
      ...
      if (existingSubscription) {
        throw this.createError(
          "SUBSCRIPTION_001",
          "Client already has an active subscription with this brand",
          400
        );
      }
      ...
      const subscription = new Subscription({
        ...
        currentPeriodEnd: this.calculateCurrentPeriodEnd(
          new Date(),
          subscriptionPlan.billingCycle
        ),
        frequencyUsed: 0,
        frequencyResetDate: this.calculateFrequencyResetDate(
          new Date(),
          subscriptionPlan.frequencyLimit
        ),
        autoRenew: true,
      });
      ...
      // Create PaymentIntent with Stripe (Cyprus/EUR configuration)
      const paymentIntent = await this.stripe.paymentIntents.create({
        ...
        // Re-enable transfer_data for Cyprus-based Connect accounts
        transfer_data: {
  ...
  async createCreditPaymentIntent(
    ...
    try {
      ...
      const planQuery = CreditPlan.findById(request.creditPlanId).populate(
        "brand"
      );
      if (session) planQuery.session(session);
  ...
  async confirmPayment(
    ...
    try {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        ["canceled", "payment_failed"].includes(paymentIntent.status)
      ) {
        // Payment failed
  ...
  private async completeSubscriptionPayment(
    payment: IPayment,
    session?: mongoose.ClientSession
  ): Promise<void> {
  ...
  private async completeCreditPayment(
    payment: IPayment,
    session?: mongoose.ClientSession
  ): Promise<void> {
  ...
  private calculateCurrentPeriodEnd(
    startDate: Date,
    billingCycle: string
  ): Date {
    return this.calculateSubscriptionEndDate(startDate, billingCycle);
  ...
  private calculateFrequencyResetDate(
    startDate: Date,
    frequencyLimit: any
  ): Date {
    ...
    if (frequencyLimit.period === "week") {
      const daysUntilReset =
        (frequencyLimit.resetDay - startDate.getDay() + 7) % 7;
