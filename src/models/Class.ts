...
ClassSchema.pre('save', function(next) {
  ...
  for (const block of this.timeBlocks) {
    ...
    if (startMinutes >= endMinutes) {
      return next(new Error(`Invalid time block for ${block.day}: end time must be after start time`));
    }
    
    // Check if duration matches time block duration
    const blockDuration = endMinutes - startMinutes;
    if (blockDuration !== this.duration) {
      return next(new Error(`Time block duration (${blockDuration} minutes) must match class duration (${this.duration} minutes)`));
    }
  }
  next();
});
...
ClassSchema.methods.isAvailableAt = function(day: string, time: string): boolean {
